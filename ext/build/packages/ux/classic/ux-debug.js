...
Ext.define('Ext.ux.ajax.GroupingSimlet', {extend:'Ext.ux.ajax.JsonSimlet', alias:'simlet.grouping', lastPost:null, lastResponse:null, doGet:function(ctx) {
  var me = this, data = me.getData(ctx), page = me.getPage(ctx, data), reader = ctx.xhr.options.proxy && ctx.xhr.options.proxy.getReader(), root = reader && reader.getRootProperty(), ret = me.callParent(arguments), response = {}, summary = {}, i, len, s;
  if (root && Ext.isArray(page)) {
    response[root] = page;
    response[reader.getTotalProperty()] = data.length;
  } else {
  ...
  var me = this, params = ctx.params, order = (params.filter || '') + (params.group || '') + '-' + (params.sort || '') + '-' + (params.dir || ''), tree = me.tree, data, fields, sortFn, filters;
  if (tree) {
    me.fixTree(ctx, tree);
  }
  me.lastPost = params;
  ...
  var groupers = ctx.groupSpec, summaries = ctx.summarySpec, out = [], i, j, k, len, length, len2, grouper, record, values, summary, keys;
  len = groupers.length;
  for (i = 0; i < len; i++) {
    grouper = groupers[i];
...
Ext.define('Ext.ux.event.Player', function(Player) {
  ...
  return {extend:'Ext.ux.event.Driver', keyFrameEvents:{click:true}, pauseForAnimations:true, speed:1, stallTime:0, _inputSpecialKeys:{INPUT:inputSpecialKeys, TEXTAREA:Ext.apply({}, inputSpecialKeys)}, tagPathRegEx:/(\w+)(?:\[(\d+)\])?/, xpathRe:/^[#~](?:[a-z][-a-z0-9_]*)(?:\/[a-z]+)*$/i, constructor:function(config) {
    var me = this;
    ...
    var me = this, target = me.locateElement(eventDescriptor.target), now = me.getTimeIndex(), timeout = eventDescriptor.timeout, event;
    ...
    var me = this, doneName = hookName + '.done', firedName = hookName + '.fired', hook = hookHandler || eventDescriptor[hookName];
...
Ext.define('Ext.ux.dd.BoxContainerDD', {extend:'Ext.dd.DD', alignElWithMouse:function(el, iPageX, iPageY) {
  var me = this, oCoord = me.getTargetCoord(iPageX, iPageY), x = oCoord.x, y = oCoord.y, fly = el.dom ? el : Ext.fly(el, '_dd'), aCoord, newLeft, newTop;
  if (!me.deltaSetXY) {
    aCoord = [Math.max(0, x), Math.max(0, y)];
    fly.setXY(aCoord);
...
Ext.define('Ext.ux.BoxReorderer', {extend:'Ext.plugin.Abstract', alias:'plugin.boxreorderer', requires:['Ext.ux.dd.BoxContainerDD'], mixins:{observable:'Ext.util.Observable'}, itemSelector:'.x-box-item', animate:100, constructor:function() {
  this.callParent(arguments);
  ...
  var me = this, dragCmp = me.dragCmp, targetEl, dom, left, top, scrollable;
  if (dragCmp) {
    ...
    if (!dragCmp.nextSibling()) {
      ...
      me.spacerEl = Ext.dom.Helper.append(targetEl, {tag:'div', style:'width: 1px;' + 'height: 1px;' + 'position: absolute;' + 'left: ' + left + 'px;' + 'top: ' + top + 'px;"'});
    }
  } else {
    me.dragElId = null;
  }
  ...
  var me = this, items = me.container.items, container = me.container, direction = me.curIndex < newIndex ? 1 : -1, orig, dest, tmpIndex;
  ...
  var me = this, dragCmp = me.dragCmp, container = me.container, layout = container.getLayout(), temp;
  if (e) {
    e.stopEvent();
  }
  if (dragCmp) {
  ...
  var me = this, spacerEl = Ext.fly(me.spacerEl), dragCmp = me.dragCmp;
...
Ext.define('Ext.ux.CellDragDrop', {extend:'Ext.plugin.Abstract', alias:'plugin.celldragdrop', uses:['Ext.view.DragZone'], enforceType:false, applyEmptyText:false, emptyText:'', dropBackgroundColor:'green', noDropBackgroundColor:'red', dragText:'{0} selected row{1}', ddGroup:'GridDD', enableDrop:true, enableDrag:true, containerScroll:false, init:function(view) {
  ...
  if (me.enableDrop) {
    me.dropZone = new Ext.dd.DropZone(view.el, {view:view, ddGroup:me.dropGroup || me.ddGroup, containerScroll:true, getTargetFromEvent:function(e) {
      ...
      var self = this, destType, sourceType;
...
Ext.ux.DataViewTransition = Ext.extend(Object, {defaults:{duration:750, idProperty:'id'}, constructor:function(config) {
  ...
  dataview.store.on('datachanged', function(store) {
    var parentEl = dataview.getTargetEl(), calcItem = store.getAt(0), added = this.getAdded(store), removed = this.getRemoved(store), previous = this.getRemaining(store);
    ...
    var el = Ext.get(this.dataviewID + '-' + calcItem.get(this.idProperty)), itemWidth = el.getMargin('lr') + el.getWidth(), itemHeight = el.getMargin('bt') + el.getHeight(), dvWidth = parentEl.getWidth(), columns = Math.floor(dvWidth / itemWidth);
    ...
    Ext.iterate(store.data.items, function(item) {
      var id = item.get(idProperty), column = index % columns, row = Math.floor(index / columns), top = row * itemHeight, left = column * itemWidth;
      newPositions[id] = {top:top, left:left};
      index++;
    }, this);
    var startTime = new Date(), duration = this.duration, dataviewID = this.dataviewID, doAnimate = function() {
      var elapsed = new Date() - startTime, fraction = elapsed / duration, id;
      if (fraction >= 1) {
    ...
    }, task = {run:doAnimate, interval:20, scope:this};
...
Ext.define('Ext.ux.DataView.Animated', {alias:'plugin.ux-animated-dataview', defaults:{duration:750, idProperty:'id'}, constructor:function(config) {
  ...
  function reDraw() {
    ...
    doAnimate = function() {
      var elapsed = new Date() - task.taskStartTime, fraction = elapsed / duration, oldPos, newPos, oldTop, newTop, oldLeft, newLeft, diffTop, diffLeft, midTop, midLeft;
      if (fraction >= 1) {
  ...
  var count = 0, items = this.getExisting(), k;
  for (k in items) {
    count++;
  }
  return count;
...
Ext.define('Ext.ux.statusbar.StatusBar', {extend:'Ext.toolbar.Toolbar', xtype:'statusbar', alternateClassName:'Ext.ux.StatusBar', requires:['Ext.toolbar.TextItem'], cls:'x-statusbar', busyIconCls:'x-status-busy', busyText:'Loading...', autoClear:5000, emptyText:'\x26#160;', activeThreadId:0, initComponent:function() {
  ...
  var me = this, statusEl = me.statusEl, text, iconCls;
  config = config || {};
...
Ext.define('Ext.ux.LiveSearchGridPanel', {extend:'Ext.grid.Panel', requires:['Ext.toolbar.TextItem', 'Ext.form.field.Checkbox', 'Ext.form.field.Text', 'Ext.ux.statusbar.StatusBar'], searchValue:null, matches:[], currentIndex:null, searchRegExp:null, caseSensitive:false, regExpMode:false, matchCls:'x-livesearch-match', defaultStatusText:'Nothing Found', initComponent:function() {
  ...
  var me = this, count = 0, view = me.view, columns = me.visibleColumnManager.getColumns();
  view.refresh();
...
Ext.define('Ext.ux.Spotlight', {baseCls:'x-spotlight', animate:true, duration:250, easing:null, active:false, constructor:function(config) {
  ...
  var me = this, box = me.el.getBox(), viewWidth = Ext.Element.getViewportWidth(), viewHeight = Ext.Element.getViewportHeight(), from, to, clone;
...
Ext.define('Ext.ux.TabCloseMenu', {extend:'Ext.plugin.Abstract', alias:'plugin.tabclosemenu', mixins:{observable:'Ext.util.Observable'}, closeTabText:'Close Tab', showCloseOthers:true, closeOtherTabsText:'Close Other Tabs', showCloseAll:true, closeAllTabsText:'Close All Tabs', extraItemsHead:null, extraItemsTail:null, constructor:function(config) {
  ...
  var me = this, items;
  if (!me.menu) {
    items = [{itemId:'close', text:me.closeTabText, scope:me, handler:me.onClose}];
    if (me.showCloseAll || me.showCloseOthers) {
      items.push('-');
    }
    if (me.showCloseOthers) {
      items.push({itemId:'closeOthers', text:me.closeOtherTabsText, scope:me, handler:me.onCloseOthers});
    }
    if (me.showCloseAll) {
...
Ext.define('Ext.ux.ToolbarDroppable', {constructor:function(config) {
  ...
  var canAdd = this.canDrop(dragSource, event, data), tbar = this.toolbar, entryIndex;
  if (canAdd) {
...
Ext.define('Ext.ux.colorpick.ColorUtils', function(ColorUtils) {
  ...
  return {singleton:true, constructor:function() {
    ...
    if (el) {
      ...
      data = {hex:ColorUtils.rgb2hex(color.r, color.g, color.b), alpha:Math.floor(color.a * 255).toString(16)};
    ...
    var c = v * s, hprime = h / 60, x = c * (1 - Math.abs(hprime % 2 - 1)), rgb = [0, 0, 0], m;
    ...
    g = g / 255;
    b = b / 255;
    var M = Math.max(r, g, b), m = Math.min(r, g, b), c = M - m, hprime = 0, s = 0, h, v;
    if (c !== 0) {
      if (M === r) {
        hprime = (g - b) / c % 6;
      } else if (M === g) {
...
Ext.define('Ext.ux.colorpick.ColorMapController', {extend:'Ext.app.ViewController', alias:'controller.colorpickercolormapcontroller', requires:['Ext.ux.colorpick.ColorUtils'], onFirstBoxReady:function() {
  ...
  var me = this, vm = me.getViewModel(), rgba = vm.get('selectedColor'), container = me.getView(), dragHandle = container.down('#dragHandle'), containerEl = container.getEl(), containerWidth = containerEl.getWidth(), containerHeight = containerEl.getHeight(), hsv, xRatio, yRatio, left, top;
...
Ext.define('Ext.ux.colorpick.ColorPreview', {extend:'Ext.Component', alias:'widget.colorpickercolorpreview', requires:['Ext.util.Format', 'Ext.XTemplate'], style:'position: relative', html:'\x3cdiv class\x3d"' + Ext.baseCSSPrefix + 'colorpreview-filter" style\x3d"height:100%; width:100%; position: absolute;"\x3e\x3c/div\x3e' + '\x3ca class\x3d"btn" style\x3d"height:100%; width:100%; position: absolute;"\x3e\x3c/a\x3e', cls:Ext.baseCSSPrefix + 'colorpreview', height:256, onRender:function() {
  var me = this;
  me.callParent(arguments);
...
'Ext.ux.colorpick.SliderHue'], config:{hexReadOnly:true}, width:580, height:337, cls:Ext.baseCSSPrefix + 'colorpicker', padding:10, layout:{type:'hbox', align:'stretch'}, defaultBindProperty:'value', twoWayBindable:['value'], fieldWidth:50, fieldPad:5, showPreviousColor:false, showOkCancelButtons:false, listeners:{resize:'onResize'}, constructor:function(config) {
  ...
  if (config.showOkCancelButtons) {
    items.push({xtype:'button', text:'OK', margin:'10 0 0 0', padding:'10 0 10 0', handler:'onOK'}, {xtype:'button', text:'Cancel', margin:'10 0 0 0', padding:'10 0 10 0', handler:'onCancel'});
  }
...
Ext.define('Ext.ux.data.PagingMemoryProxy', {extend:'Ext.data.proxy.Memory', alias:'proxy.pagingmemory', alternateClassName:'Ext.data.PagingMemoryProxy', constructor:function() {
  ...
  if (filters.length > 0) {
    ...
    Ext.each(result.records, function(record) {
      var isMatch = true, length = filters.length, filter, fn, scope, i;
      for (i = 0; i < length; i++) {
        filter = filters[i];
        fn = filter.filterFn;
        scope = filter.scope;
...
Ext.define('Ext.ux.dd.CellFieldDropZone', {extend:'Ext.dd.DropZone', alias:'plugin.ux-cellfielddropzone', containerScroll:true, onCellDrop:Ext.emptyFn, constructor:function(cfg) {
  ...
  var me = this, v = me.view, cell = e.getTarget(v.getCellSelector());
  if (cell) {
...
'\x3cdiv class\x3d"ux-desktop-shortcut" id\x3d"{name}-shortcut"\x3e', '\x3cdiv class\x3d"ux-desktop-shortcut-icon {iconCls}"\x3e', '\x3cimg src\x3d"', Ext.BLANK_IMAGE_URL, '" title\x3d"{name}"\x3e', '\x3c/div\x3e', '\x3cspan class\x3d"ux-desktop-shortcut-text"\x3e{name}\x3c/span\x3e', '\x3c/div\x3e', '\x3c/tpl\x3e', '\x3cdiv class\x3d"x-clear"\x3e\x3c/div\x3e'], taskbarConfig:null, windowMenu:null, initComponent:function() {
  ...
  var me = this, availWidth = me.body.getWidth(true), x = me.xTickSize, y = me.yTickSize, nextY = y;
  me.windows.each(function(win) {
    var w;
...
Ext.define('Ext.ux.desktop.TaskBar', {extend:'Ext.toolbar.Toolbar', requires:['Ext.button.Button', 'Ext.resizer.Splitter', 'Ext.menu.Menu', 'Ext.ux.desktop.StartMenu'], alias:'widget.taskbar', cls:'ux-taskbar', startBtnText:'Start', initComponent:function() {
  ...
  var config = {iconCls:win.iconCls, enableToggle:true, toggleGroup:'all', width:140, margin:'0 2 0 3', text:Ext.util.Format.ellipsis(win.title, 20), listeners:{click:this.onWindowBtnClick, scope:this}, win:win}, cmp = this.windowBar.add(config);
  cmp.toggle(true);
  return cmp;
...
'\x3c/tpl\x3e', '{html}', '\x3c/video\x3e'], initComponent:function() {
  var me = this, fallback, cfg, chrome;
  if (me.fallbackHTML) {
    ...
    if (Ext.isChrome) {
      ...
      chrome = '\x3ca href\x3d"http://www.google.com/chrome"\x3eChrome\x3c/a\x3e';
  ...
  var me = this, video = me.video, videoDom;
  video = me.video;
...
Ext.define('Ext.ux.event.RecorderManager', {extend:'Ext.panel.Panel', alias:'widget.eventrecordermanager', uses:['Ext.ux.event.Recorder', 'Ext.ux.event.Player'], layout:'fit', buttonAlign:'left', eventsToIgnore:{mousemove:1, mouseover:1, mouseout:1}, bodyBorder:false, playSpeed:1, initComponent:function() {
  ...
  var me = this, idle = !me.player && !me.recorder.active, view;
...
Ext.define('Ext.ux.grid.TransformGrid', {extend:'Ext.grid.Panel', constructor:function(table, config) {
  ...
  var configFields = config.fields || [], configColumns = config.columns || [], fields = [], cols = [], headers = table.query('thead th'), i = 0, len = headers.length, data = table.dom, width, height, col, text, name;
  for (; i < len; ++i) {
    col = headers[i];
    text = col.innerHTML;
    name = 'tcol-' + i;
...
Ext.define('Ext.ux.layout.ResponsiveColumn', {extend:'Ext.layout.container.Auto', alias:'layout.responsivecolumn', states:{small:1000, large:0}, _responsiveCls:Ext.baseCSSPrefix + 'responsivecolumn', initLayout:function() {
  ...
  if (Ext.isIE8) {
    Responsive.override({responsiveSizePolicy:{readsWidth:0, readsHeight:0, setsWidth:1, setsHeight:0}, setsItemSize:true, calculateItems:function(ownerContext, containerSize) {
      var me = this, targetContext = ownerContext.targetContext, items = ownerContext.childItems, len = items.length, gotWidth = containerSize.gotWidth, contentWidth = containerSize.width, i, itemContext, itemMarginWidth, itemWidth;
      if (gotWidth === false) {
