...
Ext.define('Ext.data.soap.Reader', {extend:'Ext.data.reader.Xml', alias:'reader.soap', getData:function(data) {
  var envelope = data.documentElement, prefix = envelope.prefix;
  return Ext.DomQuery.selectNode(prefix + '|Body', data);
}});
Ext.define('Ext.data.soap.Proxy', {extend:'Ext.data.proxy.Ajax', alias:'proxy.soap', requires:['Ext.data.soap.Reader'], config:{soapAction:{}, operationParam:'op', reader:'soap', url:'', envelopeTpl:['\x3c?xml version\x3d"1.0" encoding\x3d"utf-8" ?\x3e', '\x3csoap:Envelope xmlns:soap\x3d"http://schemas.xmlsoap.org/soap/envelope/"\x3e', '{[values.bodyTpl.apply(values)]}', '\x3c/soap:Envelope\x3e'], createBodyTpl:null, readBodyTpl:['\x3csoap:Body\x3e', '\x3c{operation} xmlns\x3d"{targetNamespace}"\x3e', 
'\x3ctpl foreach\x3d"params"\x3e', '\x3c{$}\x3e{.}\x3c/{$}\x3e', '\x3c/tpl\x3e', '\x3c/{operation}\x3e', '\x3c/soap:Body\x3e'], updateBodyTpl:null, destroyBodyTpl:null, writeBodyTpl:['\x3csoap:Body\x3e', '\x3c{operation} xmlns\x3d"{targetNamespace}"\x3e', '\x3ctpl for\x3d"records"\x3e', '{% var recordName\x3dvalues.modelName.split(".").pop(); %}', '\x3c{[recordName]}\x3e', '\x3ctpl for\x3d"fields"\x3e', '\x3c{name}\x3e{[parent.get(values.name)]}\x3c/{name}\x3e', '\x3c/tpl\x3e', '\x3c/{[recordName]}\x3e', 
'\x3c/tpl\x3e', '\x3c/{operation}\x3e', '\x3c/soap:Body\x3e'], targetNamespace:''}, applyEnvelopeTpl:function(tpl) {
  ...
  var me = this, action = operation.getAction(), soapOperation = me.getApi()[action], params = Ext.applyIf(operation.getParams() || {}, me.getExtraParams() || {}), xmlData = me.getEnvelopeTpl().apply({operation:soapOperation, targetNamespace:me.getTargetNamespace(), params:params, records:operation.getRecords(), bodyTpl:me.getBodyTpl(action)}), request = new Ext.data.Request({url:me.getUrl() + '?' + me.getOperationParam() + '\x3d' + soapOperation, method:'POST', action:action, operation:operation, 
  xmlData:xmlData, headers:Ext.apply({SOAPAction:me.getSoapAction()[action]}, me.getHeaders()), timeout:me.getTimeout(), scope:me, disableCaching:false});
