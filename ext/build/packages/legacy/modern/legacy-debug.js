...
Ext.define('Ext.device.filesystem.HTML5', {extend:'Ext.device.filesystem.Abstract', requestFileSystem:function(config) {
  ...
  var me = this, successCallback = function(fs) {
  ...
  Ext.define('Ext.device.filesystem.FileSystem', {fs:null, root:null, constructor:function(fs) {
    ...
    Ext.define('Ext.device.filesystem.DirectoryEntry', {extend:'Ext.device.filesystem.Entry', cachedDirectory:null, constructor:function(path, fileSystem) {
      ...
      var me = this, callback = config.success;
      ...
      var me = this, fullPath = this.path + config.path, fileEntry = Ext.create('Ext.device.filesystem.FileEntry', fullPath, this.fileSystem);
      ...
      var me = this, fullPath = this.path + config.path, directoryEntry = Ext.create('Ext.device.filesystem.DirectoryEntry', fullPath, this.fileSystem);
    ...
    Ext.define('Ext.device.filesystem.FileEntry', {extend:'Ext.device.filesystem.Entry', length:0, offset:0, constructor:function(path, fileSystem) {
      ...
      var me = this, originalConfig = Ext.applyIf({}, config);
      if (this.fileSystem) {
...
Ext.define('Ext.device.notification.Simulator', {extend:'Ext.device.notification.Abstract', requires:['Ext.MessageBox', 'Ext.util.Audio'], msg:null, show:function() {
  ...
  var animation = ['@-webkit-keyframes vibrate{', '    from {', '        -webkit-transform: rotate(-2deg);', '    }', '    to{', '        -webkit-transform: rotate(2deg);', '    }', '}', 'body {', '    -webkit-animation: vibrate 50ms linear 10 alternate;', '}'], head = document.getElementsByTagName('head')[0], cssNode = document.createElement('style');
